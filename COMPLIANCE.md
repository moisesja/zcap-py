...
> - **0.9.0** — #11 (invocation is a `capabilityInvocation` Data-Integrity proof over the target; `proof.invocationTarget` parsed + bound + optional `expected_target` request binding; `proof.capability` string-or-embedded), #23 (legacy `invoker` removed — controller-only), #28 (`proofPurpose` re-asserted at verify time + `did:key` verification-relationship seam). **#14 closed**: a genuine `@digitalbazaar/zcap` known-answer fixture (`tests/fixtures/digitalbazaar/`) now verifies under the default URDNA2015 verifier, so the `proofHash || docHash` byte-parity assumption is locked to a real external answer (no longer "believed, pending KAT"). The bundled `zcap-v1` / `ed25519-2020` contexts are guarded byte-identical to digitalbazaar's.
...
> The scorecard reflects status through **0.9.0**. Rows for closed issues are updated with current
> `file:line` evidence; rows for still-open issues may retain the original audit snapshot — consult the
> linked issue for authoritative current status.
...
| Current spec does NOT define a capability "type" field; it must not be required/expected | SHOULD | 🟡 | `invocation no longer requires a body type (parser.py parse_invocation); delegated cap still tolerates optional type via _validate_optional_type("Authorization")` | Invocation body `type` no longer mandated (#11 done); the optional delegated `type:"Authorization"` tolerance remains a minor carry | [#11](https://github.com/moisesja/zcap-py/issues/11) |
| Legacy "invoker" field is NOT part of the current spec (controller only) | SHOULD | ✅ | `models.py Capability has no invoker; parser.py drops it; invocation.py:_controller_contains(capability.controller, …) controller-only` | Legacy invoker removed (0.9.0, #23) — identity is controller-only, matching @digitalbazaar/zcap | [#23](https://github.com/moisesja/zcap-py/issues/23) |
...
| Requirement | Level | Status | Evidence | Gap | Issue |
|---|---|---|---|---|---|
| Invocation MUST be a Data Integrity proof (proofPurpose=capabilityInvocation) over the target; spec defines NO standalone type:"Invocation" object | MUST | ✅ | `parser.py parse_invocation requires a capabilityInvocation proof, no body type; verified end-to-end by the genuine digitalbazaar KAT (test_kat_digitalbazaar.py)` | Proof-over-target model (0.9.0, #11); a real @digitalbazaar/zcap invocation parses & verifies | [#11](https://github.com/moisesja/zcap-py/issues/11) |
| Body-level capability/invocationTarget should not be duplicated; carried in the proof | SHOULD | ✅ | `Invocation.capability/.invocation_target are properties over proof; no body duplication; consistency checks removed` | Carried only in the proof (0.9.0, #11) | [#11](https://github.com/moisesja/zcap-py/issues/11) |
| proof.invocationTarget MUST be parsed and cross-checked against the capability/request target | MUST | ✅ | `LinkedDataProof.invocation_target; parser requires proof.invocationTarget; invocation.py binds it to the verified-leaf target + optional expected_target (request)` | Signed proof.invocationTarget parsed and bound (0.9.0, #11); malicious-proof-target gap closed | [#11](https://github.com/moisesja/zcap-py/issues/11) |
...
| Concatenation order MUST be proofHash \|\| docHash, byte-identical to @digitalbazaar/jsonld-signatures createVerifyData | MUST | ✅ | `ed25519_2020_w3c.py proof_hash+doc_hash; LOCKED by a genuine @digitalbazaar/zcap fixture verifying under our verifier (test_kat_digitalbazaar.py) + bundled-context byte-identity guard` | Byte parity proven against a real digitalbazaar proof (0.9.0, #14) — no longer believed-pending | [#14](https://github.com/moisesja/zcap-py/issues/14) |
| A public W3C URDNA2015 signing API MUST exist (library must PRODUCE W3C proofs, not only verify) | MUST | ✅ | `ed25519_2020_w3c.py sign_document_proof_w3c, exported from __init__; round-trips with verify_document_proof_w3c and the digitalbazaar KAT` | Public W3C signer (0.7.0); KAT confirms output is digitalbazaar-shaped (#14) | [#14](https://github.com/moisesja/zcap-py/issues/14) |
...
| W3C proof options MUST carry the ed25519-2020 suite context so proof terms expand identically to digitalbazaar | MUST | ✅ | `ed25519_2020_w3c.py _build_proof_options raises ProofError unless the document @context includes the ed25519-2020 suite; KAT confirms identical N-Quads vs digitalbazaar` | Suite context required (0.8.0); byte parity confirmed by the KAT (#14) | [#14](https://github.com/moisesja/zcap-py/issues/14) |
...
| Signer MUST be authorized for the parent controller's capabilityDelegation relationship; proofPurpose MUST be honored at verify time | MUST | 🟡 | `delegation.py _verify_delegation_link asserts proof_purpose==capabilityDelegation + calls the did:key relationship authorizer before crypto; signer==parent.controller string check` | Verify-time proofPurpose + did:key relationship gate landed (0.9.0, #28); full controller-document resolution for non-did:key still pending | [#28](https://github.com/moisesja/zcap-py/issues/28) |
...
| Invocation proof proofPurpose MUST be capabilityInvocation, validated at verify time | MUST | ✅ | `invocation.py verify_invocation asserts proof_purpose==capabilityInvocation before crypto, independent of the parser; delegation.py asserts capabilityDelegation` | Re-asserted at verify time (0.9.0, #28) — a wrong-purpose pre-built model is rejected | [#28](https://github.com/moisesja/zcap-py/issues/28) |
...
| Invocation signer MUST equal capability controller (current spec; invoker is legacy) | MUST | ✅ | `invocation.py controller-only check (legacy invoker removed)` | Controller-only identity (0.9.0, #23) | [#23](https://github.com/moisesja/zcap-py/issues/23) |
| Proof key MUST be authorized by controller for the correct verification relationship (capabilityDelegation/capabilityInvocation) | MUST | 🟡 | `relationships.py did_key_relationship_authorizer + verify-time proofPurpose gate in delegation.py/invocation.py; seam pluggable via ZcapVerifier(relationship_authorizer=…)` | Verify-time relationship gate + did:key seam landed (0.9.0, #28); full non-did:key controller-document resolution still pending | [#28](https://github.com/moisesja/zcap-py/issues/28) |
